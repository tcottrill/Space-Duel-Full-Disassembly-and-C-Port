 ...
 * this file was reverse-engineered against (several ROM addresses here
 * carry Atari names - LANG/DIFF/SAUMIN/INTRPT/SYNC/GAME/ATRACT/MAXSPE -
 * that are pure zero-page-reuse coincidences with no bearing on their
 * eponymous meanings; raw hex is used instead of those macros so the code
 * doesn't lie about what it is doing).
 */
#include "sd_state.h"
#include "sd_hw.h"
#include "sd_bcd.h"
 ...
 *           still needs X reloads it from XCOMP/WHITE afterwards, and
 ...
 *
 * ZERO-PAGE REUSE (same trap as UpdateHighScoreTable - see NOTES_score.md):
 *   $3A,X / INTRPT+X ($3B,X) / SYNC+X ($3C,X) are the 3-byte live score
 *   for slot X (0 = right/solo player, 3 = left player); INTRPT/SYNC's
 *   eponymous meanings do not apply.  The combined-game slot is slot 6,
 *   which the ROM spells out absolutely as EAWRIT ($40) / UPDFLG ($41) /
 *   $42 - and those two EAROM-sounding labels appear NOWHERE ELSE in the
 *   whole ROM, so they are pure symbol-table artifacts.  Raw hex is used
 *   for them below so the code does not claim an EAROM meaning it has not
 *   got.  DIFF ($DB) and OPTN1 ($D9), by contrast, ARE read as themselves
...
void add_points_to_score(uint8_t a)
{
    ...
    a = r.r;
    TEMP5 = a;                              /* L5F79: kept for the combined score */
    ...
    r = bcd_adc(0x00, g.ram[A_INTRPT + x], c);  /* L5F8B-8D: score byte 1   */
    g.ram[A_INTRPT + x] = r.r;              /* L5F8F                        */
    YTOP = r.r;                             /* L5F91: saved for the bonus compare */
    r = bcd_adc(0x00, g.ram[A_SYNC + x], r.c);  /* L5F93-95: score byte 2   */
    g.ram[A_SYNC + x] = r.r;                /* L5F97                        */

    if (LASTSW & 0x80)                      /* L5F99/9B BIT/BMI: was combined */
        goto aps_65;
    if (DIFF == 0)                          /* L5F9D/9F: bonus allowed?     */
        goto aps_90;
    x = OWNER;                              /* L5FA1                        */
    a = g.ram[A_OPTN1 + x];                 /* L5FA4: this player's next bonus */
    if (a != YTOP)                          /* L5FA6/A8: cannot pass on one hit */
        goto aps_90;
    r = bcd_adc(a, DIFF, 0);                /* L5FAA/AB: step to next level */
    g.ram[A_OPTN1 + x] = r.r;               /* L5FAD                        */
...
aps_60:                                     /* AddPointsToScore_60 ($5FBE)  */
    if (!(LASTSW & 0x80))                   /* L5FBE/C0 BPL: not combined   */
        goto aps_90;
    /* falls into _65 */

aps_65:                                     /* AddPointsToScore_65 ($5FC2)  */
    CMBSCFLAG--;                            /* L5FC2: tell the IRQ it changed */
    r = bcd_adc(TEMP5, g.ram[0x40], 0);     /* L5FC5-C8: combined byte 0    */
    g.ram[0x40] = r.r;                      /* L5FCA ("EAWRIT" = slot 6 + 0) */
    if (!r.c)                               /* L5FCC BCC _90                */
        goto aps_90;
    r = bcd_adc(0x00, g.ram[0x41], r.c);    /* L5FCE-D0: combined byte 1    */
    g.ram[0x41] = r.r;                      /* L5FD2 ("UPDFLG" = slot 6 + 1) */
    r = bcd_adc(0x00, g.ram[0x42], r.c);    /* L5FD4-D6: combined byte 2    */
    g.ram[0x42] = r.r;                      /* L5FD8                        */
    if (DIFF == 0)                          /* L5FDA/DC: no bonus allowed   */
        goto aps_90;
    a = OPTN1;                              /* L5FDE: player 0's is used for combined */
    if (a != g.ram[0x41])                   /* L5FE0/E2: achieved?          */
        goto aps_90;
    r = bcd_adc(a, DIFF, 0);                /* L5FE4/E5: step to next level */
    OPTN1 = r.r;                            /* L5FE7                        */
...
aps_80:                                     /* AddPointsToScore_80 ($5FF8)  */
     ...
     * in TEMP5 (overwriting the point value staged above) and parks the
     * caller's Y in TEMP6.  Y is unknowable from this signature - see
 ...
 *     TEMP1 ($10, already set by the caller: 1 for the game0-left call,
 ...
 * The live score being compared is NOT named cells despite the operand
 * text: $3A,x / INTRPT+x ($3B,x) / SYNC+x ($3C,x) are zero-page bytes
 * reused here as a 3-byte scratch score staged by the caller before
 * calling UpdateHighScoreTable - INTRPT/SYNC's eponymous meanings do not
 * apply in this context (see NOTES_score.md). Likewise LANG/DIFF/SAUMIN
 * ($DA/$DB/$DC) are read only as "$DD/$DE/$DF minus 3" address arithmetic
 * for the shift-down copy, never as their own named values. */
static void jmp_upda20(uint8_t x)
{
    ...
    TEMP2 = FIFTH_VALUE_UPDATE_CHECK(game + 1);  /* L6646-9: end boundary  */
    ...
    for (;;) {
         ...
         * vs the live score (LSB $3A,x .. MSB SYNC+x); only the final
        ...
        uint32_t score = ((uint32_t)g.ram[A_SYNC + x] << 16)
                        | ((uint32_t)g.ram[A_INTRPT + x] << 8)
                        |  (uint32_t)g.ram[0x3A + x];
        if (table < score)
            break;                        /* L665E BCC: NEW HIGH SCORE    */
        y = (uint8_t)(y + 3);             /* L6660-2 INY x3               */
        if (y >= TEMP2)                   /* L6663/5 CPY TEMP2/BCC        */
    ...
    POKRAN = x;                           /* L6668                        */
    POTGO = y;                            /* L666A                        */
    g.ram[0x38 + TEMP1] = y;              /* L666C-6F: ZP_38/ZP_39 (TYA/STA $38,X) */
    g.ram[A_FLSFLG + TEMP1] = y;          /* L6671                        */
    ...
    while (idx != y) {
        ...
        g.ram[0xDD + idx] = g.ram[0xDA + idx];  /* L6698-B: LANG as $DD-3 */
        g.ram[0xDE + idx] = g.ram[0xDB + idx];  /* L669E-A1: DIFF as $DE-3*/
        g.ram[0xDF + idx] = g.ram[0xDC + idx];  /* L66A4-7: SAUMIN as $DF-3*/
    ...
    ZP_45 = 0xED;                          /* L66E6-8                      */
    g.ram[0xDF + POTGO] = g.ram[A_SYNC + POKRAN];    /* L66EA-F0          */
    g.ram[0xDE + POTGO] = g.ram[A_INTRPT + POKRAN];  /* L66F3-5           */
    g.ram[0xDD + POTGO] = g.ram[0x3A + POKRAN];      /* L66F8-A           */

    /* UpdateUpdateHighScore_28/_29 ($66FD): player-2 cancellation, only
     * for the non-special (TEMP1==0, SPFLG==$FF) case. */
    if (TEMP1 == 0 && (SPFLG & 0x80) && !(ZP_39 & 0x80) && ZP_39 >= ZP_38) {
 ...
 * steps). In: a = binary value. Out: NOBJ = low 2 BCD digits, g.ram[0x16]
 * = BCD hundreds digit (unnamed scratch, always 0 for the credit counts
 * this is actually used on); YTOP is clobbered (shift register). Call
...
void hex_bcd_conversion_input(uint8_t a)
{
    ...
    YTOP = a;                             /* L8C62: shift register        */
    NOBJ = 0;                             /* L8C66-8                      */
    /* SED (L8C6C): decimal mode for the 8 double-and-add steps. */
    for (i = 0; i < 8; i++) {             /* L8C64: LDY #$07 .. DEY/BPL   */
        int carry = (YTOP & 0x80) != 0;   /* L8C6D: ASL YTOP, carry out   */
        bcd_res r;
        YTOP = (uint8_t)(YTOP << 1);
        r = bcd_adc(NOBJ, NOBJ, carry);   /* L8C6F-73                     */
        NOBJ = r.r;
...
void display_4_names(void)
{
    ...
    YTOP = 3;                             /* L7529-B: display 4 names     */
    ...
    for (;;) {
        ...
        if (TEMP8 != 0 && !(TEMP7B & 0x80) && !(STRTLOK & 0x80)) {
            y = FLASHCOL;                              /* L7570             */
        ...
        y = 0xC7;                                        /* L75B7             */
        if (TEMP8 != 0) {                                /* L75B9/BB           */
            if (TEMP7B & 0x80)                            /* L75C1/3           */
                goto d4n_17;
            if (!(STRTLOK & 0x80) && YTOP == ZP_34)       /* L75C5/8, CA/CE    */
                goto d4n_30;
            ...
            if (TEMP2 & 0x80)                               /* L75DB/D           */
                goto d4n_30;
            if (TEMP8 >= 2)                                 /* L75DF-E3          */
                goto d4n_30;
            if (YTOP & 0x01)                                /* L75E5-8: odd = 1-player */
                goto d4n_30;
            x = 2;                                          /* L75EA             */
            goto d4n_16;
        }
    ...
    if (TEMP8 != 0) {                      /* L766B/D                      */
        int carry;
        hex_bcd_conversion_input(TEMP8);    /* L766F                        */
        carry = display_digit_with_zero((uint8_t)(NOBJ >> 4), 1); /* L7672-9 */
        (void)display_digit_with_zero(NOBJ, carry);              /* L767C-E */
    }
    if (TEMPB != 0)                         /* L7681/3: any half-credit?    */
